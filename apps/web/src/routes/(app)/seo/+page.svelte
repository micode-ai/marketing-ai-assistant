<script lang="ts">
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await api.get<any[]>('/seo/keywords', params);
      items = res;
    } catch (e) {
      ...
      {#each items as item}
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="font-medium text-gray-900 dark:text-white">{item.keyword}</h3>
              <p class="text-sm text-gray-500 dark:text-gray-400">{item.intent || ''}{item.volume != null ? ' · Vol: ' + item.volume : ''}</p>
            </div>
