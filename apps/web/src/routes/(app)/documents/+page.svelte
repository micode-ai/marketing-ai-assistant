...
<div class="p-4 sm:p-6">
  ...
  <div class="flex items-center justify-between mb-6">
    ...
    {#if $currentProjectStore}
      <a href="/projects/{$currentProjectStore.id}/documents" class="inline-flex items-center gap-2 px-4 py-2 bg-brand text-white text-sm font-medium rounded-lg hover:brightness-110 transition-colors">
        + {$_('common.create')}
      </a>
    {:else}
  ...
  {:else}
    <div class="space-y-3">
      {#each items as item}
        <a href="/projects/{item.projectId}/documents" class="block bg-surface rounded-lg border border-border p-4 hover:border-brand/40 hover:shadow-sm transition-all cursor-pointer">
