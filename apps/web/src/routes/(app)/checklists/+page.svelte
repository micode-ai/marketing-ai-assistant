...
<div class="p-4 sm:p-6">
  ...
  <div class="flex items-center justify-between mb-6">
    ...
    {#if $currentProjectStore}
      <a href="/projects/{$currentProjectStore.id}/checklists" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors">
        + {$_('common.create')}
      </a>
    {:else}
  ...
  {:else}
    <div class="space-y-3">
      {#each items as item}
        <a href="/projects/{item.projectId}/checklists" class="block bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-sm transition-all cursor-pointer">
