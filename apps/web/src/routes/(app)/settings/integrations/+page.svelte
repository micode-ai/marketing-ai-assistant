<script lang="ts">
  ...
  // and the user needs to know that before scheduling anything.
  let tiktokDirectPost: boolean | null = null;
  ...
  onMount(async () => {
    ...
    // Best-effort: the section still renders if this fails.
    try {
      const caps = await api.get<{ configured: boolean; directPost: boolean }>('/tiktok/capabilities');
      tiktokDirectPost = caps.directPost;
    } catch {
    ...
    if (params.get('instagram') === 'connected') {
      ...
      showToast($_('social.tiktok.connected'), 'success');
      history.replaceState(null, '', window.location.pathname);
    } else if (params.get('tiktok') === 'error') {
      const reason = params.get('reason');
      showToast(
        reason === 'no_tiktok_account'
          ? $_('social.tiktok.noAccount')
          : reason === 'access_denied'
            ? $_('social.tiktok.accessDenied')
            : $_('social.tiktok.error'),
        'error',
      );
      history.replaceState(null, '', window.location.pathname);
    }
  ...
  async function connectTikTok() {
    try {
      const result = await api.get<{ url: string }>('/tiktok/auth-url', { organizationId: $organizationIdStore });
      window.location.href = result.url;
    } catch (e: any) {
      showToast(e.message || $_('social.tiktok.error'));
    }
  }
...
<div class="p-4 sm:p-6 max-w-4xl">
  ...
  {:else}
    ...
    <div class="bg-surface rounded-xl border border-border p-5 mb-3">
      <div class="flex items-center gap-3 mb-3">
        <div class="w-10 h-10 rounded-lg {platformColor['TIKTOK']} flex items-center justify-center flex-shrink-0">
          {@html platformIcon['TIKTOK']}
        </div>
        <div>
          <div class="font-medium text-ink">TikTok</div>
          <div class="text-xs text-ink-muted">{$_('social.tiktok.description')}</div>
        </div>
      </div>
      {#if tiktokDirectPost === false}
        <!-- Until the TikTok posting audit passes, published content can only land
             in the creator's drafts — say so instead of letting users wonder. -->
        <p class="text-xs text-amber-700 bg-amber-500/10 border border-amber-500/30 rounded-lg p-2.5 mb-3">
          {$_('social.tiktok.draftsMode')}
        </p>
      {/if}
      <div class="flex flex-col gap-2">
        {#each accounts.filter(a => a.platform === 'TIKTOK') as account}
          <div class="flex flex-col gap-2 p-2.5 rounded-lg border border-border bg-surface-2/50">
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2 min-w-0">
                {#if account.profileImageUrl}
                  <img src={account.profileImageUrl} alt={account.accountName} class="w-7 h-7 rounded-full flex-shrink-0" />
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                ...
                {#if account.status === 'REAUTH_REQUIRED'}
                  <button on:click={connectTikTok} class="text-xs px-3 py-1.5 border border-orange-400 text-orange-700 bg-orange-500/20 rounded-lg font-medium transition-colors duration-150 cursor-pointer">
        ...
        {/each}
        <button on:click={connectTikTok} class="text-sm px-4 py-2 bg-[#FE2C55] text-white rounded-lg hover:bg-[#e02549] transition-colors duration-150 cursor-pointer font-medium self-start mt-1">
          {accounts.some(a => a.platform === 'TIKTOK') ? $_('social.addAnother') : $_('social.tiktok.connect')}
        </button>
      </div>
    </div>
