<script lang="ts">
  ...
  // Brave Search state
  interface BraveConfig { configured: boolean; lastValidationError: string | null }
  let braveConfig: BraveConfig | null = null;
  let braveLoading = false;
  let braveApiKey = '';
  let braveConnecting = false;
  let braveDisconnecting = false;
  let braveError = '';
  let braveSuccess = '';
  let braveShowReconnectForm = false;
  ...
  onMount(async () => {
    ...
    // Fetch Brave Search config
    await fetchBraveConfig();
  });
  ...
  async function fetchBraveConfig() {
    braveLoading = true;
    try {
      braveConfig = await api.get<BraveConfig>(`/seo/brave/config/${projectId}`);
    } catch {
      braveConfig = { configured: false, lastValidationError: null };
    } finally {
      braveLoading = false;
    }
  }

  function braveValidationErrorMessage(code: string | null): string {
    if (!code) return '';
  ...
  async function connectBrave() {
    if (!braveApiKey.trim()) return;
    braveConnecting = true;
    braveError = '';
    try {
      await api.post('/seo/brave/config', { projectId, apiKey: braveApiKey.trim() });
      braveSuccess = $_('seo.braveConfig.connectSuccess');
      setTimeout(() => { braveSuccess = ''; }, 5000);
      braveApiKey = '';
      braveShowReconnectForm = false;
      await fetchBraveConfig();
    } catch (e: any) {
      braveError = e.message || 'Failed to connect';
      setTimeout(() => { braveError = ''; }, 5000);
    } finally {
      braveConnecting = false;
    }
  }

  async function disconnectBrave() {
    braveDisconnecting = true;
    braveError = '';
    try {
      await api.delete(`/seo/brave/config/${projectId}`);
      braveConfig = { configured: false, lastValidationError: null };
      braveSuccess = $_('seo.braveConfig.disconnect') + ' ✓';
      setTimeout(() => { braveSuccess = ''; }, 3000);
    } catch (e: any) {
      braveError = e.message || 'Failed to disconnect';
      setTimeout(() => { braveError = ''; }, 5000);
    } finally {
      braveDisconnecting = false;
    }
  }
</script>
...
&lt;script src="{trackingInfo.snippetUrl}"&gt;&lt;/script&gt;</pre>
  ...
  <div class="bg-white rounded-xl border border-gray-200 p-5 mt-6">
    ...
    {:else if braveConfig?.configured && !braveShowReconnectForm}
      <!-- Connected state -->
      <div class="space-y-4">
        <div class="flex items-center gap-3">
          <div class="w-3 h-3 rounded-full {braveConfig.lastValidationError ? 'bg-red-500' : 'bg-green-500'}"></div>
          <span class="text-sm font-medium text-gray-900">{$_('seo.braveConfig.connected')}</span>
        </div>

        <div class="space-y-1">
          <div class="flex items-center gap-2">
            <span class="text-xs font-medium text-gray-500">{$_('seo.braveConfig.apiKey')}:</span>
            <code class="text-sm bg-gray-50 px-2 py-0.5 rounded border border-gray-200 font-mono">••••••••</code>
          </div>
        </div>

        {#if braveConfig.lastValidationError}
          <div class="p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
            <p class="mb-2">
              {$_('seo.braveConfig.validationError', { values: { error: braveValidationErrorMessage(braveConfig.lastValidationError) } })}
            </p>
            <button
              on:click={() => { braveShowReconnectForm = true; }}
              class="text-sm font-medium text-red-700 underline hover:no-underline cursor-pointer"
            >
              {$_('seo.braveConfig.reconnect')}
            </button>
          </div>
        {/if}
        ...
        <div class="flex items-center gap-3">
          <button
            on:click={disconnectBrave}
            disabled={braveDisconnecting}
          ...
          >
            {braveDisconnecting ? $_('common.loading') : $_('seo.braveConfig.disconnect')}
          </button>
        </div>
      </div>
    {:else}
      ...
      <div class="space-y-4">
        ...
        <div class="border border-gray-200 rounded-lg p-4 space-y-3">
          <div>
            <label class="block text-xs font-medium text-gray-600 mb-1" for="brave-api-key">{$_('seo.braveConfig.apiKey')}</label>
            <input
              id="brave-api-key"
              type="password"
          ...
          <div class="flex items-center gap-3">
            <button
              on:click={connectBrave}
              disabled={braveConnecting || !braveApiKey.trim()}
            ...
            >
              {braveConnecting ? $_('common.loading') : $_(braveShowReconnectForm ? 'seo.braveConfig.reconnect' : 'seo.braveConfig.connect')}
            </button>
            {#if braveShowReconnectForm}
              <button
                on:click={() => { braveShowReconnectForm = false; braveApiKey = ''; }}
