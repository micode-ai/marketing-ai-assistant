<script lang="ts">
  ...
  // across /projects/A → /projects/B, so onMount does NOT fire again.
  let mounted = false;
  ...
  async function reloadForProjectChange() {
    // Reset cached per-tab state so everything refetches for the newly selected project.
    utmData = [];
    funnelData = null;
    pagesData = [];
    dailyData = [];
  ...
  onMount(async () => {
    ...
    await detectSurfaces();
    // Web data is lazy — loaded when user first activates the 'website' channel.
  ...
  function switchPeriod(period: number) {
    ...
    // AnalyticsOverview and channel dashboards react via their `days` prop.
    // Re-fetch website data only if it has already been loaded (lazy pattern).
    if (webLoaded) {
      fetchData();
  ...
  async function switchChannel(ch: ChannelId) {
    activeChannel = ch;
    if (ch === 'website') {
      if (!webLoaded) {
        await ensureWebData();
      } else if (dailyData.length > 0 && ChartJS) {
        // Re-render charts when switching back to website tab (canvases were removed from DOM).
        await tick();
  ...
  $: kpiCards = [
    ...
    },
    {
      ...
      change: totals?.change?.conversions ?? 0, trend: totals?.trend?.conversions ?? 'stable',
      hintKey: 'hints.metric.conversionRate',
  ...
  // Website channel sub-tab definitions
  const webSubTabs = [
  ...
  // Compute visible channel tabs from connection flags
  $: channelTabs = (() => {
    const tabs: { id: ChannelId; labelKey: string }[] = [
      { id: 'overview', labelKey: 'analytics.tabOverviewChannel' },
    ];
...
<div class="p-4 sm:p-6">
  ...
  {:else}
    <!-- Page header + single global period selector -->
    ...
    <!-- Summary strip: collapsed AnalyticsOverview always visible above channel tabs -->
    <div class="mb-6">
      <AnalyticsOverview
        projectId={projectId ?? ''}
        days={selectedPeriod}
    ...
    <div class="flex border-b border-border mb-6">
      {#each channelTabs as tab}
        <button on:click={() => switchChannel(tab.id)}
          class="px-4 py-2.5 text-sm font-medium border-b-2 transition-colors duration-150 -mb-px cursor-pointer
            {activeChannel === tab.id ? 'border-primary-600 text-brand' : 'border-transparent text-ink-muted hover:text-ink'}">
          {$_(tab.labelKey)}
        </button>
      {/each}
    </div>
    ...
    <!-- Channel body -->
    {#if activeChannel === 'overview'}
      <!-- Expanded overview: combo chart + channel quick-links -->
      <AnalyticsOverview
        projectId={projectId ?? ''}
        days={selectedPeriod}
        expanded={true}
      ...
      {:else if activeTab === 'overview'}
        ...
        {:else}
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {#each kpiCards as card}
              <div class="bg-surface rounded-xl border border-border p-4 border-t-4 {card.borderColor}">
                ...
                <div class="text-xs text-ink-muted mt-1 flex items-center gap-1">
          ...
          <div class="bg-surface rounded-xl border border-border p-5 mb-6">
            <h3 class="text-sm font-semibold text-ink mb-4 flex items-center gap-1.5">
              {$_('analytics.trafficOverview')}
              <InfoTooltip key="hints.metric.overviewTrend" />
            </h3>
          ...
          <div class="bg-surface rounded-xl border border-border p-5 mb-6">
            <h3 class="text-sm font-semibold text-ink mb-4 flex items-center gap-1.5">
              {$_('analytics.emailPerformance')}
              <InfoTooltip key="hints.metric.emailPerformance" />
            </h3>
