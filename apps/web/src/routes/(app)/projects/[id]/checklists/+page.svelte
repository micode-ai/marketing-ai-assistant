        ...
        {#each checklists as cl}
          ...
          >
            ...
            <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium
              {clProgress === 100 ? 'bg-green-500/20 text-green-700' : 'bg-surface-2 text-ink-muted'}">
              {clProgress}%
            </span>
          </button>
        {/each}
            ...
            <div class="flex items-center sm:items-start gap-3 flex-shrink-0">
              ...
              {#if deletingId === checklist.id}
                <div class="flex items-center gap-1">
                  <button on:click={() => deleteChecklist(checklist.id)} class="px-3 py-1.5 rounded-lg bg-red-500/12 text-red-600 hover:bg-red-500/20 transition text-xs font-medium" title={$_('common.delete')}>
              ...
              {:else}
                <button on:click={() => deletingId = checklist.id} class="p-1.5 rounded-lg text-ink-subtle hover:text-red-500 hover:bg-red-500/12 transition" title={$_('common.delete')}>
              ...
              <div class="rounded-lg transition-colors {expandedItems.has(item.id) ? 'bg-surface-2' : 'hover:bg-surface-2/50'}" on:mouseenter={() => hoveredItemId = item.id} on:mouseleave={() => { if (hoveredItemId === item.id) hoveredItemId = null; }}>
                <div class="flex items-center gap-3 py-2 px-2">
                  ...
                  <div class="flex items-center gap-0.5">
                    {#if itemIndex > 0}
                      <button on:click|stopPropagation={() => moveItem(checklist.id, items, itemIndex, 'up')} class="p-1 rounded text-ink-subtle hover:text-ink-muted hover:bg-surface-2" title={$_('checklists.moveUp')}>
                    ...
                    {#if itemIndex < items.length - 1}
                      <button on:click|stopPropagation={() => moveItem(checklist.id, items, itemIndex, 'down')} class="p-1 rounded text-ink-subtle hover:text-ink-muted hover:bg-surface-2" title={$_('checklists.moveDown')}>
                    ...
                    {#if deletingItemId === item.id}
                      <button on:click|stopPropagation={() => deleteItem(checklist.id, item.id)} class="p-1 rounded bg-red-500/12 text-red-600 hover:bg-red-500/20">
                      ...
                      <button on:click|stopPropagation={() => deletingItemId = null} class="p-1 rounded text-ink-subtle hover:text-ink-muted hover:bg-surface-2">
                    ...
                    {:else}
                      <button on:click|stopPropagation={() => deletingItemId = item.id} class="p-1 rounded text-ink-subtle hover:text-red-500 hover:bg-red-500/12" title={$_('checklists.deleteItem')}>
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                      </button>
                    {/if}
                  </div>
                  {/if}
                  <div class="w-2 h-2 rounded-full flex-shrink-0 {priorityDot[item.priority] || 'bg-border-strong'}" title={item.priority}></div>
                </div>
...
{#if showImportModal}
  ...
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" on:click|self={() => showImportModal = false}>
    <div class="bg-surface rounded-2xl shadow-2xl w-full max-w-md p-6">
      ...
      {#if importError}
        <div class="text-sm text-red-600 mb-4 bg-red-500/12 p-3 rounded-lg">{importError}</div>
      {/if}
          ...
          <ul class="space-y-1">
            {#each importParsed.items as item}
              <li class="flex items-center gap-2 text-sm">
                ...
                <div class="w-1.5 h-1.5 rounded-full flex-shrink-0 {priorityDot[item.priority] || 'bg-border-strong'}"></div>
              </li>
            {/each}
          </ul>
        </div>
