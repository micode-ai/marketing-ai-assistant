...
4. Транзакция в БД создаёт:
   - Запись User
   - Запись Organization (план FREE)
   - Запись OrganizationMember (роль: OWNER)
   - Запись Subscription (статус: trialing, 14 дней)

5. Генерируются JWT-токены
