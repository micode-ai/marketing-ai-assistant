...
Генерирует чек-листы маркетинговых задач с подробными описаниями.
...
**Поведение:**
- Генерирует 25-35 пунктов, сгруппированных по 4-6 секциям
- Каждый пункт содержит описание на 6-10 предложений: конкретные шаги, инструменты, метрики, ошибки
- Приоритеты: CRITICAL (4-6), HIGH (8-10), MEDIUM, LOW
- `maxTokens: 16384`, температура: 0.3
- Поддержка языка через `getLanguageInstruction(language)`
- LangGraph: `loadContext → generateChecklist → parseJson → [fixJson] → saveChecklist`
- Сохраняет Checklist + ChecklistItem записи в БД

### Document Agent (Документы)

**Файл:** `apps/ai-agent/src/agents/document-agent.ts`

Генерирует маркетинговые документы в Markdown-формате.
...
```json
{
  "type": "REPORT",
  "language": "ru",
  "title": "Отчёт об эффективности",
  "context": "дополнительный контекст"
}
```

**Поведение:**
- `maxTokens: 8192`, температура: 0.4
- Для типа `REPORT` загружает реальные данные проекта из БД:
  - Контент (статусы, типы, последние записи)
  - Кампании (статусы)
  - Подписчики email (количество)
  - Подключённые соц. аккаунты
  - Чек-листы (% выполнения задач)
- ИИ получает инструкцию: использовать **только реальные данные**, не придумывать метрики
- Если данных нет — честно указывает «нет данных» и рекомендует что настроить
- `PRODUCT_HUNT_BRIEF` включает: tagline, описание, комментарий мейкера, посты для дня запуска

### Шаблоны (Templates Page)

Страница `/templates` использует агентов CHECKLIST и DOCUMENT для генерации шаблонов:
- Фронтенд отправляет `POST /agent/run` с `agentType`, `input.type`, `input.language` (из `$locale`)
- Поллит `GET /agent/runs/:id` каждые 2 секунды до статуса `COMPLETED`
- Показывает оверлей генерации с анимацией (30-60 секунд)
- После завершения редиректит на страницу чек-листов/документов
