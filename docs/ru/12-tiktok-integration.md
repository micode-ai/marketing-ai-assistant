...
2. Редирект на `www.tiktok.com/v2/auth/authorize/`, всегда с `disable_auto_auth=1`.
   По умолчанию TikTok пропускает страницу авторизации, если в браузере уже есть живая
   сессия, и молча привязывает *этот* аккаунт — то есть при подключении второго аккаунта
   пользователь получит первый, не увидев ни какой это аккаунт, ни какие права выданы.
   Плюс без этого параметра экран согласия невозможно снять для App Review после того,
   как приложение уже было авторизовано один раз.
